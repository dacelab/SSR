! 
!  This driver program is called by dacelib top level routine
!  dace_setup for maximum likelihood estimation
!
!  In Dacelib 1.2 the gradients are evaluated by finite differencing
!
!  DRIVER ROUTINE FOR L-BFGS-B
!
...
!
!  Modified by 
!               Prasanth Nair Nov 1998
!
!***WARNING*** This routine should not be compiled with aggressive compiler optimization flags
!  Recommended flags: -O
!

      subroutine LBFGSDriver
     .          (ndata, nvar, ldb, Vars, funcvec, itermax, n, x, 
     .           lda, Rtheta, work, temp1, temp2, wts, iregress)
...
!
      implicit none 
      integer          nmax, mmax, lda, ldb, iregress
...
! A description of all these variables is given at the end of the
! driver.
!
! Optimizer Variables
!
     ...
     +                 x(*), l(nmax), u(nmax), g(nmax), dsave(29), 
     +                 wa(2*mmax*nmax+4*nmax+12*mmax*mmax+12*mmax),
     +                 gradf(nmax)

      double precision t1, t2, sig2
      integer          i, ii

!
! Dacelib variables
!
      integer          ndata, nvar, itermax
      double precision Vars(ldb,*), funcvec(*),
     .                 beta, Rtheta(lda,*), wts(*),
     .                 work(*), temp1(*), temp2(*), fd_step
!
! Local variables
!
      integer          count, igrad
      double precision f1, x1(nmax), xold(nmax), nrm


      iprint =  -1    ! We suppress the default output.

      igrad =  0     ! FD gradients

      fd_step = 0.01 ! FD step-size (%)
...
! set bounds on the odd numbered variables.

      do 10 i=1,n
         nbd(i)=1   !assume only a lower bound exists
         l(i)=0.001
!        u(i)=1.0d2
  10  continue
...
!  Compute function value f for the sample problem.
         ...
         call DaceFcn
     .       (n, x, f, ndata, nvar, ldb, Vars, funcvec,
     .        lda, Rtheta, beta, work, temp1, temp2, wts, sig2,
     .        iregress)
         count = count + 1
...
!            call Dace_grad
!    .           (n, x, gradf, ndata, nvar, ldb, Vars, funcvec,
!    .           lda, Rtheta, beta, work, temp1, temp2, wts, sig2,
!    .            iregress)
...
!
!  Compute gradients g using forward finite differencing
!
         do i = 1, n
            do ii=1,n
               x1(ii) = x(ii)
            end do
            x1(i) = x(i) + fd_step*x(i)

            call DaceFcn
     .          (n, x1, f1, ndata, nvar, ldb, Vars, funcvec,
     .           lda, Rtheta, beta, work, temp1, temp2, wts, sig2,
     .           iregress)
            count = count + 1

            g(i) = (f1 - f)/(fd_step*x(i))
         end do
         endif

         do i = 1,n
            xold(i) = x(i)  ! Update xold
         end do
...
!        if (count .ge.itermax) 
         ...
         if (isave(30) .ge.itermax) 
     +      task='STOP: TOTAL NO. of f AND g EVALUATIONS EXCEEDS LIMIT'

c        2) Terminate if  |proj g|/(1+|f|) < 1.0d-4, where 
c           "proj g" denoted the projected gradient

         if (dsave(13) .le. 1.0d-4*(1.0d0 + abs(f)))
