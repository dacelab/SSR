...
c     --------------------------------------------------------------
      ...
      subroutine LBFGSDriver(x, n, Ydata_train, N_train, rank, d, 
     .          basissize, varphi_train, lbda, f, factr, pgtol, optf)
...
!
! l1-penalized minimization variables
!     
      integer          itermax, N_train, rank, d, basissize
      double precision lbda, fd_step, Ydata_train(N_train),
...
c     ------- Grand loop for optimization ----------
 
      do i = 1,n
         xold(i) = x(i)
      end do
      count = 0 ! counts number of function evaluations
      count_it = 0 ! counts the number of iterations
      f = 0.0d0
 111  continue
...
c        Compute function value f
         call evalFcn(n,x,f,N_train,rank,d,basissize,lbda,
...
c        Compute gradient g using FORWARD finite differencing
c         do i = 1, n
c            do ii = 1,n
c               x1(ii) = x(ii)
c            end do
c            x1(i) = x(i) + fd_step*x(i)
c     
c            call evalFcn(n,x1,f1,N_train,rank,d,basissize,lbda,
c     +                   varphi_train,Ydata_train)
c      
c            count = count + 1
c        
c            g(i) = (f1 - f)/(fd_step*x(i))
c         end do
         
c        Compute gradient g using CENTERED finite differencing
c         do i = 1, n
c            do ii = 1,n
c               x1(ii) = x(ii)
c               x2(ii) = x(ii)
c           end do
c            x1(i) = x(i) + fd_step*x(i)
c            x2(i) = x(i) - fd_step*x(i)
c     
c            call evalFcn(n,x1,f1,N_train,rank,d,basissize,lbda,
c     +                   varphi_train,Ydata_train)
c     
c            call evalFcn(n,x2,f2,N_train,rank,d,basissize,lbda,
c     +                   varphi_train,Ydata_train)
c     
c            count = count + 2
c        
c            g(i) = 0.5d0*(f1 - f2)/(fd_step*x(i))
c         end do
         
c        Compute gradient explicitly         
         call evalGradFcn(n,x,g,N_train,rank,d,basissize,lbda,
     +                    varphi_train,Ydata_train)  
...
c      print*, 'Total number of iterations=', count_it
  
      end
      
c======================= The end of BFGSDriver =============================
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c LBFGSDriver_block is used for solving the subproblems in the block
c version of SRCD.
c
c      on entry
c
c      x             double precision(n)
c                    x contains a block of variables corresponding
c                    to a fixed dimension.
c
c      n             integer
c                    n equals rank*basissize.
c
c     comp1d_train   double precision(rank,d,N_train)
c                    comp1d_train contains all the component functions 
c                    evaluated at the training points. Needed for 
c                    computing the objective/gradient functions.
c
c     blocknb        integer
c                    index of block of variables (or dimension) 
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

      subroutine LBFGSDriver_block(x, n, Ydata_train, N_train, rank, d, 
     .          basissize, varphi_train, comp1d_train, nbblock, lbda, f, 
     .          factr, pgtol, optf)
...
! l1-penalized minimization variables
!     
      integer          itermax, N_train, rank, d, basissize, nbblock
      double precision lbda, fd_step, Ydata_train(N_train),
     +                 varphi_train(N_train,basissize,d),
...
c        Compute function value f
         call evalFcn_block(n,x,f,N_train,rank,d,basissize,lbda,
     +    varphi_train,comp1d_train,nbblock,Ydata_train)
         if (optf .eq. 1) then 
            print*,'f=', f
	 endif   
         count = count + 1
...
c            end do
c            x1(i) = x(i) + fd_step*x(i)
c             call evalFcn_block(n,x1,f1,N_train,rank,d,basissize,lbda,
c     +    varphi_train,comp1d_train,nbblock,Ydata_train)
c      
c            count = count + 1
c        
c            g(i) = (f1 - f)/(fd_step*x(i))
...
c     +    varphi_train,comp1d_train,nbblock,Ydata_train)
c
c            call evalFcn_block(n,x2,f2,N_train,rank,d,basissize,lbda,
c     +    varphi_train,comp1d_train,nbblock,Ydata_train)
c     
c            count = count + 2
c        
...
c        Compute gradient explicitly
         call evalGradFcn_block(n,x,g,N_train,rank,d, 
     + basissize,lbda,varphi_train,comp1d_train,nbblock,Ydata_train)
