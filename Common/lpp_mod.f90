...
!
  ...
  call i4_choose ( n + k - 1, n, number )

  return
end subroutine comp_enum
...
!
  ...
  call i4_uniform_ab ( rank1, rank2, seed, rank )

  call comp_unrank_grlex ( kc, rank, xc )

  return
end subroutine comp_random_grlex
...
!
  ...
  do i = 1, ks
    ...
    if ( tim1 + 1 <= xs(i) - 1 ) then
      do j = tim1 + 1, xs(i) - 1
        call i4_choose ( ns - j, ks - i, i4_choose_out )
        rank = rank + i4_choose_out
      end do
    end if
  ...
  do n = 0, nc - 1
    call i4_choose ( n + kc - 1, n, i4_choose_out )
    rank = rank + i4_choose_out
  end do

  return
end subroutine comp_rank_grlex
...
!
  ...
  call i4_choose ( ns, ks, nksub )

  j = 1

  do i = 1, ks

    call i4_choose ( ns - j, ks - i, r )
...
!
  ...
  return
end subroutine comp_unrank_grlex

subroutine i4_choose ( n, k, i4_choose_out )
...
!
  ...
  return
end subroutine i4_choose

subroutine i4_uniform_ab ( a, b, seed, i4_uniform_ab_out )
...
!
  ...
  integer ( kind = 4 ) i4_uniform_ab_out
  integer ( kind = 4 ) k
  real ( kind = 4 ) r
  integer ( kind = 4 ) seed
  integer ( kind = 4 ) value
...
subroutine i4vec_sum ( n, a, i4vec_sum_out )
...
!
  ...
  integer ( kind = 4 ) a(n)
  integer ( kind = 4 ) i4vec_sum_out

  i4vec_sum_out = sum ( a(1:n) )

  return
end subroutine i4vec_sum
...
!
  ...
  return
end subroutine lp_coefficients
...
!
!    Output, real ( kind = 8 ) V(N), the values of the Legendre polynomials 
...
!
  ...
  if ( 1 <= o ) then
    ...
    do j = 2, o
      ...
      vtable(1:n,j) = &
        ( real ( 2 * j - 1, kind = 8 ) * x(1:n) * vtable(1:n,j-1)   &
...
!
  ...
  return
end subroutine lpp_to_polynomial
...
!
!    Output, real ( kind = 8 ) VALUE(N), the value of the Legendre Product 
...
!
  ...
  do n = 0, nm - 1
    call i4_choose ( n + m - 1, n, i4_choose_out )
    rank = rank + i4_choose_out
  end do

  return
end subroutine mono_rank_grlex
...
!
  ...
  return
end subroutine mono_unrank_grlex

subroutine mono_upto_enum ( m, n, mono_upto_enum_out )
...
!
  ...
  integer ( kind = 4 ) m
  integer ( kind = 4 ) mono_upto_enum_out
  integer ( kind = 4 ) n
  integer ( kind = 4 ) value

  call i4_choose ( n + m, n, value )

  mono_upto_enum_out = value

  return
end subroutine mono_upto_enum
...
!
  ...
  return
end subroutine mono_upto_next_grlex
...
!
  ...
  rank_min = 1
  call mono_upto_enum ( m, n, rank_max )
  call i4_uniform_ab ( rank_min, rank_max, seed, rank )
  call mono_unrank_grlex ( m, rank, x )

  return
end subroutine mono_upto_random
...
!
  ...
  return
end subroutine polynomial_compress
...
!      
  ...
  return
end subroutine polynomial_print
...
!
  ...
  return
end subroutine polynomial_sort
...
!
  ...
  return
end subroutine polynomial_value
